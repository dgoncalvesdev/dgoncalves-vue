...
var _parseVars = function _parseVars2(params, type, parent) {
  var isLegacy = _isNumber(params[1]), varsIndex = (isLegacy ? 2 : 1) + (type < 2 ? 0 : 1), vars = params[varsIndex], i;
  if (isLegacy) {
...
var _slice = [].slice;
var _isArrayLike = function _isArrayLike2(value) {
...
var Timeline = function(_Animation) {
  ...
  _proto2.render = function render(totalTime, suppressEvents, force) {
    var prevTime = this._time, tDur = this._dirty ? this.totalDuration() : this._tDur, dur = this._dur, tTime = totalTime > tDur - _tinyNum && totalTime >= 0 && this !== _globalTimeline ? tDur : totalTime < _tinyNum ? 0 : totalTime, crossingStart = this._zTime < 0 !== totalTime < 0 && this._initted, time, child, next, iteration, cycleDuration, prevPaused, pauseTween, timeScale, prevStart, prevIteration, yoyo, isYoyo;
    if (tTime !== this._tTime || force || crossingStart) {
      ...
      if (tTime === tDur || !tTime && this._ts < 0) {
        if (prevStart === this._start || Math.abs(timeScale) !== Math.abs(this._ts)) {
          if (!time || tDur >= this.totalDuration()) {
    ...
    if (startAt) {
      ...
      if (prevStartAt) {
        ...
        _removeFromParent(tween._startAt = Tween.set(targets, _merge(_copyExcluding(vars, _reservedProps), {
          ...
          stagger: 0,
          parent
          //ensures that nested tweens that had a stagger are handled properly, like gsap.from(".class", {y:gsap.utils.cycle([-100,100])})
        })));
        if (!immediateRender) {
          _initTween2(tween._startAt, time);
          if (immediateRender) {
            !autoRevert && (tween._startAt = 0);
          }
        } else if (!time) {
...
var _staggerTweenProps = _callbackNames + ",repeat,repeatDelay,yoyo,yoyoEase";
var _staggerPropsToSkip = (_staggerTweenProps + ",id,stagger,delay,duration").split(",");
var Tween = function(_Animation2) {
  ...
  function Tween2(targets, vars, time) {
    ...
    var _this4$vars = _this4.vars, duration = _this4$vars.duration, delay = _this4$vars.delay, immediateRender = _this4$vars.immediateRender, stagger = _this4$vars.stagger, overwrite = _this4$vars.overwrite, keyframes = _this4$vars.keyframes, defaults2 = _this4$vars.defaults, parsedTargets = toArray(targets), tl, i, copy, l, p, curTarget, staggerFunc, staggerVarsToMerge;
...
_forEachName("staggerTo,staggerFrom,staggerFromTo", function(name) {
  Tween[name] = function() {
    var tl = new Timeline(), params = toArray(arguments);
...
var _renderPlain = function _renderPlain2(ratio, data) {
  return data.set(data.t, data.p, ~~((data.s + data.c * ratio) * 1e4) / 1e4, data);
};
...
var _renderComplexString = function _renderComplexString2(ratio, data) {
  ...
  if (!ratio && data.b) {
    ...
    while (pt) {
      s = pt.p + (pt.m ? pt.m(pt.s + pt.c * ratio) : ~~((pt.s + pt.c * ratio) * 1e4) / 1e4) + s;
      pt = pt._next;
    }
    s += data.c;
  }
...
var _renderCSSProp = function _renderCSSProp2(ratio, data) {
  return data.set(data.t, data.p, ~~((data.s + data.c * ratio) * 1e4) / 1e4 + data.u, data);
};
var _renderPropWithEnd = function _renderPropWithEnd2(ratio, data) {
  return data.set(data.t, data.p, ratio === 1 ? data.e : ~~((data.s + data.c * ratio) * 1e4) / 1e4 + data.u, data);
};
var _renderCSSPropWithBeginning = function _renderCSSPropWithBeginning2(ratio, data) {
  return data.set(data.t, data.p, ratio ? ~~((data.s + data.c * ratio) * 1e4) / 1e4 + data.u : data.b, data);
};
...
var _getComputedProperty = function _getComputedProperty2(target, property) {
...
var _checkPropPrefix = function _checkPropPrefix2(property, element) {
  var e = element || _tempDiv, s = e.style, i = 5, a = "O,Moz,ms,Ms,Webkit".split(",");
  if (property in s) {
    return property;
  }
...
var _convertToUnit = function _convertToUnit2(target, property, value, unit) {
  var curValue = parseFloat(value), curUnit = (value + "").substr((curValue + "").length) || "px", style = _tempDiv.style, horizontal = _horizontalExp.test(property), isRootSVG = target.tagName.toLowerCase() === "svg", measureProperty = (isRootSVG ? "client" : "offset") + (horizontal ? "Width" : "Height"), amount = 100, toPixels = unit === "px", px, parent, cache, isSVG;
...
var _addRawTransformPTs = function _addRawTransformPTs2(plugin, transforms, target) {
  var style = _tempDivStyler.style, startCache = target._gsap, endCache, p, startValue, endValue, startNum, endNum, startUnit, endUnit;
